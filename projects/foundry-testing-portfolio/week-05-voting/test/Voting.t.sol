...
contract VotingTest is Test {
    ...
    function testGasBatchVsSingleRegistration() public {
        ...
        uint256 beforeReg = vm.snapshotState();
        ...
        if (batchGas <= singleGas) {
            console2.log("Gas saved (batch):", singleGas - batchGas);
        } else {
            console2.log("Extra gas (batch):", batchGas - singleGas);
        }
    ...
    function testEventsEmitted() public {
        ...
        emit ProposalCreated(
            0, voters[0], "Test Proposal", block.timestamp, block.timestamp + VOTING_PERIOD
        );
    ...
    function _getProposalData(uint256 proposalId)
        internal
        view
        returns (
            uint256 id,
